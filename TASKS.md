...
| B1 cross-major guard | Pending after correction | Verify workflow now connects the PG17 client to an isolated PG18.6 service and asserts PostgreSQL’s real `server version mismatch` refusal before export | The guard is supplied by the matching upstream client path; a new CI run is required after the test assertion correction |
